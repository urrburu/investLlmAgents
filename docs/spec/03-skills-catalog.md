...
| 부작용 | 기본은 stateless. 저장이 필요한 Skill은 저장 대상과 revision ID를 명시 |
...
| `write_internal` | 내부 캐시나 벡터 저장소에 기록 | `embed_chunks` |

`write_internal` 이상의 Skill은 실행 로그에 입력 요약, 저장 위치, 되돌릴 수 있는 방법을 남긴다.
